...
#define STORAGE_FULL_KEY    "sys.storage.full"
/**
  * sys.storage.full=[0,1]
  * returns 1 when storage is full, otherwise 0.
  */
static int is_storage_full() {
    int len = 0;
    char buf[PROPERTY_VALUE_MAX];
    len = property_get(STORAGE_FULL_KEY, buf, "0");
    if (len == 1) {
        return buf[0] == '1';
    }
    return 0;
